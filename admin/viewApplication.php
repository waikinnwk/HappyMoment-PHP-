<?php 
	...
	//$appList = getApplicationListSearch($pagenum,"N");
	$count = getCountApplicationListSearch("N");
	$p1 = $count / 20;
	...
	if(isset($_POST['oldAppId']) && $_POST['oldAppId'] != "")
		$oldAppId = $_POST['oldAppId'];
	else if(isset($_GET['oldAppId']) && $_GET['oldAppId'] != "")
		$oldAppId = $_GET['oldAppId'];	
...
<head>
	...
	<script type="text/javascript">
		<?php 
			 if(isset($_GET['emailStatus']) && $_GET['emailStatus'] == "F")
		...
		function markStatus(id,status){
			...
			if(confirm(confirmMsg)){
				...
				document.getElementById("searchParam").value = concatSearchOrderByParam().replace("searchParam=","");
		...
		function editApplication(applicationId){
			window.location = "http://" + "<?php echo $_SERVER['HTTP_HOST']."/admin/editApplication.php?applicationId=" ?>" +  applicationId + "&" + concatSearchOrderByParam();
		}
		function changeClass(applicationId){
			window.location = "http://" + "<?php echo $_SERVER['HTTP_HOST']."/admin/changeClass.php?applicationId=" ?>" + applicationId + "&" + concatSearchOrderByParam();
		}
		function concatSearchOrderByParam(){
			...
			if(document.getElementById("oldAppId").value != '')
				searchParam += ",oldAppId|" + document.getElementById("oldAppId").value;					
...
<form action='viewApplication.php' method='POST' id="listview">
<table>
	<tr>
		<td>Class Id : </td>
	...
	<tr>
		<td>Old App Id : </td>
		<td><input type="text" name="oldAppId" id="oldAppId" value="<?php echo $oldAppId; ?>"  ></td>
	</tr>	
	<tr>
...
<div class="listViewAdmin" >
	<?php getApplicationListSearchTable($pagenum,"N"); ?>
</div>
	<input type="hidden" id="action" name="action" value=""/>
	<input type="hidden" id="viewOld" name="viewOld" value="N"/>
	<input type="hidden" id="searchParam" name="searchParam" value=""/>
	<input type="hidden" id="orderBy" name="orderBy" value="<?php echo $orderBy ?>"/>
	<input type="hidden" id="currentPage" name="currentPage" value="viewApplication.php"/>
</form>
